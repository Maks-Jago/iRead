...
open class FREpubParser: NSObject, SSZipArchiveDelegate {
    ...
    open func parseCoverImage(_ epubPath: String, unzipPath: String? = nil) throws -> UIImage {
        ...
        guard let image = UIImage(contentsOfFile: coverImage.fullHref) else {
            throw FRParseError.invalidImage(path: coverImage.fullHref)
        }
    ...
    open func readEpub(epubPath withEpubPath: String, removeEpub: Bool = true, unzipPath: String? = nil) throws -> FRBook {
        ...
        if let path = unzipPath, fileManager.fileExists(atPath: path) {
            ...
            bookBasePath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)[0]
        }
    ...
    private func readContainer(with bookBasePath: String) throws {
        ...
        guard let fullPath = xmlDoc.root["rootfiles"]["rootfile"].attributes["full-path"] else {
            throw FRParseError.fullPathEmpty
        }
